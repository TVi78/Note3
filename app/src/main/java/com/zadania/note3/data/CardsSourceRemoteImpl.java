...
public class CardsSourceRemoteImpl implements CardsSource {
    ...
    @Override
    public CardsSource init(final CardSourceResponse cardSourceResponse) {
        collectionReference.orderBy(CardDateTranslate.Fields.DATE, Query.Direction.DESCENDING).get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            cardsData = new ArrayList<CardData>();
                            for (QueryDocumentSnapshot docField :
                                    task.getResult()) {
                                CardData cardData = CardDateTranslate.documentToCardData(docField.getId(),
                                        docField.getData());
    ...
    public void updateCardData(int position, CardData newCardData) {
        collectionReference.document(cardsData.get(position).getId()).
                set(CardDateTranslate.cardDataToDocument(newCardData));
    }

    @Override
    public void addCardData(CardData newCardData) {
        collectionReference.add(CardDateTranslate.cardDataToDocument(newCardData));
    }
